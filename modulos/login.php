...
<div style="margin-right: 30px;right: 0%;position: absolute;">
	<?php require('php/login.php');?>
	<?php
	if(isset($_GET['action']) && $_GET['action'] == 'joined'){
		echo "<h2>Te has registrado correctamente, verifica tu email para activar tu cuenta.</h2>";
		}?>
	<?php
						if(isset($_GET['action'])){
						switch ($_GET['action']) {
						case 'active':
						echo "<h2>La cuenta se encuentra activada ahora <h2>";
						break;
						}}
						?>
						
							<?php if(isset($error)){foreach($error as $error){echo '<p class="error-registro">'.$error.'</p>';}}?>
						</div>
